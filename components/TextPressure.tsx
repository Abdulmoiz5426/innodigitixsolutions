...
const dist = (a: MousePos, b: MousePos) => {
  const dx = b.x - a.x;
  const dy = b.y - a.y;
...
const TextPressure: React.FC<TextPressureProps> = ({
    ...
    const animate = () => {
      ...
      if (titleRef.current) {
        ...
        spansRef.current.forEach(span => {
          ...
          // Fixed readable ranges for variable font axes
  ...
  // Use a more specific class name for flex to avoid global CSS conflicts
  const dynamicClassName = [className, flex ? 'flex-container' : '', stroke ? 'stroke' : ''].filter(Boolean).join(' ');
